...
- Datos estructurados ampliados: `index.html` añade `priceRange` y `contactPoint` (teléfono) a la Organization; `servicios.html` añade un schema `Service` por cada uno de los 3 servicios; `blog.html` añade schema `Blog` con los 4 artículos. No se añadió `address` al schema a petición expresa (se queda solo en el aviso legal). Tampoco hay `AggregateRating`/`Review` porque no hay testimonios reales todavía — no inventar ese dato si se añade en el futuro, solo con reseñas reales.
